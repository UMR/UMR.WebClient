...
public partial class Oracle_DoctorAlertNetwork : System.Web.UI.Page
{
    ...
    private void PopulateControls()
    {
        ...
        {
            this.Page.Title = "Doctor Alert Network(" + dsPatient.Tables[0].Rows[0]["FirstName"].ToString() + " " + dsPatient.Tables[0].Rows[0]["LastName"].ToString() + ")";
        ...
        DataSet ds = PatientManager.GetMultipleProviderInfo(PatientKey);
        ...
        foreach (DataRow item in ds.Tables[0].Rows)
        {
            ...
            string doctorID = item["DoctorID"].ToString();
            //string desc = gv.Rows[index].Cells[1].Text;

            //Now get the details
            DataTable dsInner = PatientManager.GetProviderInfo(doctorID).Tables[0];

            foreach (DataRow innItem in dsInner.Rows)
            {
                DataRow drow = dt.NewRow();
                drow["Phone"] = innItem["CellPhone"];
                drow["Email"] = innItem["Email"];
                drow["InstitutionID"] = innItem["DoctorID"];
                drow["InstitutionName"] = innItem["FirstName"] + " " + innItem["LastName"];
                dt.Rows.Add(drow);
            }
            
        }
        
        grdInstitutions.DataSource = dt;
